...
class MessageDetailScreen extends ConsumerStatefulWidget {
  const MessageDetailScreen({this.messageId, super.key});

  /// When non-null, fetches the message by ID (e.g. from history).
  /// When null, shows the result from [messageGenerationNotifierProvider].
...
class _MessageDetailScreenState
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: LoloAppBar(
        title: l10n.messageDetail_title,
        showBackButton: true,
      ),
      body: generationState.when(
        idle: () => Center(child: Text(l10n.messageDetail_noMessage)),
        generating: () => Center(
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              const CircularProgressIndicator(color: LoloColors.colorPrimary),
              const SizedBox(height: LoloSpacing.spaceMd),
              Text(l10n.messageDetail_crafting),
            ],
          ),
        ),
      ...
      child: Column(
        ...
        children: [
          ...
          if (message.modelBadge != null)
            Text(
              l10n.messageDetail_generatedBy(message.modelBadge!),
          ...
          usageAsync.when(
            data: (usage) => Text(
              l10n.messageDetail_usageCount(usage.used, usage.limit),
              style: theme.textTheme.bodySmall?.copyWith(
                color: isDark
                    ? LoloColors.darkTextTertiary
                    : LoloColors.lightTextTertiary,
  ...
  void _copyToClipboard(BuildContext context, String content) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(AppLocalizations.of(context).messageDetail_copiedToClipboard),
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            ...
            Text(
              isFavorite ? AppLocalizations.of(context).messageDetail_savedToFavorites : AppLocalizations.of(context).messageDetail_saveToFavorites,
