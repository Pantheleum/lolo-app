...
router.post("/", async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    // Auto-create yearly anniversary reminder
    const reminderId = uuidv4();
    const reminderData = {
      title: `Memory: ${body.title}`,
      notes: null,
      category: "custom",
      date: body.date,
      time: "09:00",
      ...
      giftSuggest: false,
      partnerProfileId: memoryData.partnerProfileId || null,
    ...
    await db.collection("users").doc(req.user.uid)
      .collection("reminders").doc(reminderId).set(reminderData);
    ...
    await scheduleReminderNotifications({
      userId: req.user.uid,
      reminderId,
      eventTitle: `Memory: ${body.title}`,
      eventDate: body.date,
      eventTime: "09:00",
      tiers: [7, 1, 0],
      timezone: userTimezone,
      linkedProfileId: memoryData.partnerProfileId || null,
      linkedMemoryId: memoryId,
    });
...
router.put("/:id", async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    // If date changed, update linked reminder and reschedule notifications
    if (body.date !== undefined && body.date !== current.date) {
      const linkedReminders = await db.collection("users").doc(req.user.uid)
        .collection("reminders")
        .where("linkedMemoryId", "==", id)
        .where("deletedAt", "==", null)
        .get();

      for (const reminderDoc of linkedReminders.docs) {
        const reminderData = reminderDoc.data();
        await reminderDoc.ref.update({
          date: body.date,
          title: `Memory: ${body.title || current.title}`,
          updatedAt: new Date().toISOString(),
        });
        ...
        await rescheduleReminderNotifications({
          ...
          eventTitle: `Memory: ${body.title || current.title}`,
          eventDate: body.date,
          eventTime: reminderData.time || "09:00",
          tiers: reminderData.reminderTiers || [7, 1, 0],
          timezone: userTimezone,
          linkedProfileId: reminderData.partnerProfileId || null,
          linkedMemoryId: id,
        });
      }
    } else if (body.title !== undefined && body.title !== current.title) {
      // If only title changed, update reminder title
      const linkedReminders = await db.collection("users").doc(req.user.uid)
        .collection("reminders")
        .where("linkedMemoryId", "==", id)
        .where("deletedAt", "==", null)
...
router.delete("/:id", async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    // Cascade soft-delete linked reminders and cancel their notifications
    const linkedReminders = await db.collection("users").doc(req.user.uid)
      .collection("reminders")
      .where("linkedMemoryId", "==", id)
      .where("deletedAt", "==", null)
    ...
    for (const reminderDoc of linkedReminders.docs) {
      ...
      await cancelReminderNotifications(req.user.uid, reminderDoc.id);
    }
